#include "main.h"
/**
 * handle_print - prints argument nased on its type
...
int handle_print(const char @fmt, int *ind,
		...
		int flags, int width,
		int precisions, int size)
{
	int i, zezo_len = 0, printed_chars = -1;
	fmt_t fmt_types[] = {
	...
	for (i = 0; fmt_types[i].fmt != '\0'; i++)
		if (fmt[*ind] == fmt_types[i].fmt)
			return (fmt_types[i].fn(list, buffer, flags, width, precisions, size));
